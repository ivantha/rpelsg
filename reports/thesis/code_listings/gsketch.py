...
class BinaryPartitionTree:
    def __init__(self,
        ...
        self.countmin_tables = []  # countmin tables
        ...
        # for partitioning
        self.rem_size = INT_SIZE * self.w * self.d
    ...
    def partition(self):
        ...
        while len(stack) is not 0:
            ...
            if c2 or c1 or c3:
                ...
                idx = len(self.countmin_tables) - 1
...
class GSketch(Sketch):
    ...
    def __init__(self,
                 ...
                 w: int,  # m: Total width of the hash table
    ...
    def initialize(self):
        ...
        outlier_width = round(self.bpt.rem_size / INT_SIZE / self.d)
        if outlier_width == 0:
            print('Outlier width 0 -> 1')
            outlier_width = 1
        print('Outlier size : {:.2f}KB ({:.2f}%)'.format(INT_SIZE * outlier_width * self.d / 1024.0, outlier_width / self.w * 100.0))
        self.bpt.outlier_vertices = CountMinTable(outlier_width, self.d)

    def add_edge(self, source_id, target_id):
        if source_id in self.bpt.vertex_hash:
            self.bpt.countmin_tables[self.bpt.vertex_hash[source_id]].add_edge('{},{}'.format(source_id, target_id))
            self.partitioned_edges += 1
        else:
            self.bpt.outlier_vertices.add_edge('{},{}'.format(source_id, target_id))
            self.outlier_edges += 1

    def get_edge_frequency(self, source_id, target_id):
        if source_id in self.bpt.vertex_hash:
            return self.bpt.countmin_tables[self.bpt.vertex_hash[source_id]].get_edge_frequency(
                '{},{}'.format(source_id, target_id))
        else:
            return self.bpt.outlier_vertices.get_edge_frequency('{},{}'.format(source_id, target_id))

    @timeit
    def print_analytics(self):
        print('# partitions: {}'.format(len(self.bpt.countmin_tables)))
